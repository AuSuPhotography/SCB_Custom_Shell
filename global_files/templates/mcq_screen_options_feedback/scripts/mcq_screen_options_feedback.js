define([], function () {
	...
	McqTemplate.prototype.createOptions = function () {
				...
				$(ref.container).find(".wrapper-container-mcq").css('background-image', 'url(' + backgroundImage + ')');
				...
				$('.mcq_attempt_two').find('.sectionHeader').html(questionNo);
				$('.mcq_attempt_two').find('#qText').html(questionText);
				$('.mcq_attempt_two').find('#instruction_text').html(instructionText);
				var str = '';
				...
				str += '<div class="btn btn-info cyu-submit-btn disabled">Submit</div>';
				str += '</li>';
				str += '</ul>';
				//alert(ref.xmlName)
				//alert($(ref.container))
				$(ref.container).find('#cyu_block_two').html(str);
...
// Event for submit button functionality 
		$(ref.container).find(".cyu-submit-btn").on('click', function(){
			...
			if(ref.selectedOption == ref.correctOption){
				...
				$(ref.container).find(".popup-bg .feedback-body").find("#correct .rightDiv p").text(incorrectFeedback)	
				...
				//	$('.cyu-option:nth("'+(ref.selectedOption  - 1)+'")').find(".circle_contianer").append("<img src='assests/images/icons/wrong_2.png' style='height:21px; width:21px;' />")
					...
					$(ref.container).find(".popup-bg .feedback-body").find("#AttemptOver .rightDiv p").text(incorrectFeedback)
				    $(ref.container).find(".popup-bg").show()
