...
$(ref.container).find("#mmcq1 .wrapper-container-mmcq").css('background-image', 'url(' + backgroundImage + ')');
				var questionNo = $(ref.xml).find('questionNo').text();
				var instructionText = $(ref.xml).find('instructionText').text();
				
				$('#page7 .mmcq_attempt_one').find('#qText').html(questionText);
				$('#page7 .mmcq_attempt_one').find('#instruction_text').html(instructionText);
				$('#page7 .mmcq_attempt_one').find('.sectionHeader').html(questionNo);
				var str = '';
