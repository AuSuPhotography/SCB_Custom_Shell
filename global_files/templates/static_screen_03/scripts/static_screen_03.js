define([], function() {	
	...
	staticScreenType3.prototype.loadIntructionScreen = function() {	
		...
		for (let i = 0; i < listLength; i++) {
			str += '<li><img src=' + $(ref.xml).find('staticScreen3').find('PageContent').find('Images').eq(i).text() + '><span>'+ $(ref.xml).find('staticScreen3').find('PageContent').find('ImageText').eq(i).text() +'</span></li>'
		}
