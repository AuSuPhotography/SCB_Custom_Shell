define([], function () {
	...
	CyuMixTemplate.prototype.McqCreateOptions = function () {
		...
		str += '</ul>';
		str += '<div class="text-left submit-btn-container">' + (ref.isPrevBtn && rendomquestionNo ? '<button class="btn btn-link  prev-btn"><i class="ion-ios-arrow-left"></i>' + ref.prevBtnTxt + '</button>' : '') + '<button class="btn btn-info cyu-submit-btn disabled" role="button">Submit</button><div class="feedbackBox"></div></div>';
		// str += '</li>';
		//alert(ref.xmlName)
		//alert($(ref.container))
	...
	CyuMixTemplate.prototype.MMcqCreateOptions = function () {
		...
		for (var i = 0; i < optionLength; i++) {
			str += '<li id="opt_' + (i + 1) + '" class="_item cyu-option" data-index="' + (i) + '">'
		...
		str += '</ul>';
		str += '<div class="text-left submit-btn-container">' + (ref.isPrevBtn && rendomquestionNo ? '<button class="btn btn-link  prev-btn"><i class="ion-ios-arrow-left"></i>' + ref.prevBtnTxt + '</button>' : '') + '<button class="btn btn-info cyu-submit-btn disabled">Submit</button><div class="feedbackBox"></div></div>';
	...
	CyuMixTemplate.prototype.addMCQFunctionality = function () {
		...
		$(ref.container).find(".cyu-submit-btn").on('click', function (e) {
			...
			userInteractiionData.push(
				{
					"topic": topic,
					"questionType": questioType,
					"userSelectoption": ref.selectedOption,
					"correctAns": ref.correctOption
				}
			)
			// Send User Interation data
			window.shell.userInteractionDataSet(ref.correctOption,ref.selectedOption,questioType)
			...
			userInteractiionData.push(
				{
					"topic": topic,
					"questionType": questioType,
					"userSelectoption": encodeURI(ref.selectedOptionArray),
					"correctAns": encodeURI(ref.XMLAnswer)
				}
			)

			// Send User Interation data
			window.shell.userInteractionDataSet(encodeURI(ref.XMLAnswer),encodeURI(ref.selectedOptionArray),questioType)
			//alert(ref.selectedOption + " ---- " + ref.correctOption)
