...
// function to send User Interation data
                ...
                PageContent = function (data) {
                    ...
                    setTimeout(function () {
                        ...
                        if (ref.globalPageVisited[ref.curPage - 1] == true) {
                            ...
                            if (ref.curPage == 1 || ref.curPage == 2) {
                                $(".btnNext").removeAttr("style")
                                $(".btnNext").removeAttr("style")


                            }
                            else if (ref.currentSlideNo == ref.courseProgressArray.length - 1) {
                    ...
                    $("#slider").on("slid.bs.carousel", function () {
                        ...
                        $("#shellFooter").find("#totalTopic").text($(ref.tocXml).find('topic').find('page').length - 1)
                        $("#shellFooter").find("#currentPageTopic").text(ref.curPage - 1)
                        $("#shellFooter").show();
                        lms.unloadCourse();
                        if (ref.currentSlideNo == ref.courseProgressArray.length - 1) {
                        ...
                        if (ref.currentSlideNo == 0 || ref.currentSlideNo == 1) {
                            ...
                            if (window.globalConfig.isScorm) {
                                ...
                                ref.enableDisableloginPrevNextBtn(false)
                            }
                            return;
                        }
                        $("#page_" + ref.curTopic + "_" + ref.curPage).find('.placeHolderImage').hide();
    ...
    Shell.prototype.attachEvents = function () {
        ...
        $(".exit-popup-btn").bind('click', $.proxy(ref.openCloseExitPopup, null, ref));
    ...
    Shell.prototype.loadPopUp = function () {
        ...
        $.each(globalPopupData.find("glossaryPopupItem item"), function (k, v) {
            restr += '<tr>\
        <td>'+ $(v).find("title").text() + '</td> <td>' + $(v).find("description").text() + '</td>\
        </tr>';
        })
        ...
        $.each(globalPopupData.find("resourcePopupItem item"), function (k, v) {
            glostr += '<tr>\
            <td><a href='+ $(v).find("url").text() + ' target="_blank"> <img src="assests/images/ResourceRef.png"> ' + $(v).find("title").text() + '</a></td>\
         </tr>';
        })
    ...
    Shell.prototype.openCloseExitPopup = function (ref, e) {
            ...
            $("#slider").find(".carousel-item").each(function (k, v) {
                ...
                if (k !== 0) {
                    if ($(v).hasClass('active') && $(v).hasClass('is-completed')) {
                        menuStr += '<li class="list-group-item d-flex align-items-center complete redirecttoslide" data-page-index=' + $(v).attr("data-page") + ' data-topic=' + $(v).attr("data-topic") + '>\
                        ...
                        menuStr += '<li class="list-group-item d-flex align-items-center redirecttoslide visited" data-page-index=' + $(v).attr("data-page") + ' data-topic=' + $(v).attr("data-topic") + '>\
                        <img src="assests/images/incompleted-icon.png" class="icon-list" /><span class="title">'+ $(v).attr("data-menutitle") + '</span></li>';
                    }
                    else {
                        menuStr += '<li class="list-group-item d-flex align-items-center disabled" data-page-index=' + $(v).attr("data-page") + ' data-topic=' + $(v).attr("data-topic") + '>\
